...
class WLPostprocessor_direct(BasePostprocessor):
    ...
    def __call__(self, preds, scale):
        ...
        for cont in tr_contours:
            ...
            # level1_coeff = np.zeros((AreaNum, 20))
            # level2_coeff = np.zeros((AreaNum, 31))
            # level3_coeff = np.zeros((AreaNum, 54))
            # wl_coeff = [wl_coeff, level1_coeff, level2_coeff, level3_coeff]
            # points = pywt.waverec(wl_coeff, pywt.Wavelet('sym5'))
            ...
            # points3 = np.transpose(points.reshape(AreaNum, 2, 100), (0, 2, 1)).reshape(AreaNum, -1)
            # polygons = np.hstack((points3, score)).tolist()
            for i, (real, imag) in enumerate(zip(real_pred, imag_pred)):

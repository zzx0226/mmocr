...
class WLPostprocessor_diff_weight(BasePostprocessor):
    ...
    def __call__(self, preds, scale):
        ...
        real_preds = reg_pred[:, :, :125]
        imag_preds = reg_pred[:, :, 126:-1]

        centers_x = reg_pred[:, :, 125]
        ...
        for cont in tr_contours:
            ...
            for i, (real, imag, dxy, x, y) in enumerate(zip(real_pred, imag_pred, dxy_c, center_x, center_y)):
                ...
                wl_coeff = c * scale
                wl_coeff = [wl_coeff[:20], wl_coeff[20:40], wl_coeff[40:71], wl_coeff[71:]]

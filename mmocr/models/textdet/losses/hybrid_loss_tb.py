...
class HybridLoss_tb(nn.Module):
    def __init__(self, fourier_degree, bs_degree, cp_num, num_sample, ohem_ratio=3.):
        super().__init__()
    ...
    def forward(self, preds, _, p3_maps, p4_maps, p5_maps):
        ...
        loss_bs_x = torch.tensor(0., device=device).float()
        loss_bs_y = torch.tensor(0., device=device).float()
    ...
    def forward_single(self, pred, gt):
        ...
        bs_pred = pred[2].permute(0, 2, 3, 1).contiguous()
        ...
        bs_x_pred = bs_pred[:, :, :, 0:k1].view(-1, k1)
        bs_y_pred = bs_pred[:, :, :, k1:].view(-1, k1)
        ...
        bs_x_map = gt[:, :, :, 3 + k * 2:3 + k * 2 + k1].view(-1, k1)
        bs_y_map = gt[:, :, :, 3 + k * 2 + k1:].view(-1, k1)
        ...
        loss_bs_x = torch.tensor(0.).float().to(device)
        loss_bs_y = torch.tensor(0.).float().to(device)
        if tr_train_mask.sum().item() > 0:
            ...
            scale = 1
            loss_bs_x = torch.mean(weight * F.smooth_l1_loss(
                bs_x_map[tr_train_mask.bool()], bs_x_pred[tr_train_mask.bool()], reduction='none')) / scale
            loss_bs_y = torch.mean(weight * F.smooth_l1_loss(
                bs_y_map[tr_train_mask.bool()], bs_y_pred[tr_train_mask.bool()], reduction='none')) / scale
        return loss_tr, loss_tcl, loss_reg_x, loss_reg_y, loss_bs_x, loss_bs_y
    ...
    def fourier2poly(self, real_maps, imag_maps):
        ...
        k_vect = torch.arange(-self.fourier_degree, self.fourier_degree +
                              1, dtype=torch.float, device=device).view(-1, 1)
        i_vect = torch.arange(0, self.num_sample,
                              dtype=torch.float, device=device).view(1, -1)

        transform_matrix = 2 * np.pi / \
            self.num_sample * torch.mm(k_vect, i_vect)

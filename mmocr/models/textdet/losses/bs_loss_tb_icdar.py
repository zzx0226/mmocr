...
class BSLoss_tb_icdar(nn.Module):
    ...
    def forward_single(self, pred, gt):
        ...
        if tr_train_mask.sum().item() > 0:
            ...
            # bbox_gt = torch.cat((x_map[:, 0], y_map[:, 0], x_map[:, 3], y_map[:, 3]), dim=0).reshape(-1, 4)
            # bbox_pred = torch.cat((x_pred[:, 0], y_pred[:, 0], x_pred[:, 3], y_pred[:, 3]), dim=0).reshape(-1, 4)
            # loss_bbox = self.loss_bbox(bbox_pred[tr_train_mask.bool()], bbox_gt[tr_train_mask.bool()])
        ...
        return loss_tr, loss_tcl, loss_reg_x, loss_reg_y  #, loss_bbox  #, top_loss, bottom_loss

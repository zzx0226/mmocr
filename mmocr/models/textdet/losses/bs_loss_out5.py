...
class BSLoss_Out5(nn.Module):
    ...
    def forward(self, preds, _, p2_maps, p3_maps, p4_maps, p5_maps, p6_maps):
        """Compute FCENet loss.
        ...
        # to tensor
        gts = [p2_maps, p3_maps, p4_maps, p5_maps, p6_maps]
    ...
    def forward_single(self, pred, gt):
        ...
        if tr_train_mask.sum().item() > 0:
            weight = (tr_mask[tr_train_mask.bool()].float() +
                      tcl_mask[tr_train_mask.bool()].float()) / 20  # 10

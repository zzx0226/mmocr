    ...
    def forward_single(self, pred, gt):
        ...
        if tr_train_mask.sum().item() > 0:
            ...
            for (real_pred, real_gt, imag_pred, imag_gt, x_center_gt, y_center_gt, x_center_pred,
                ...
                wl_coeff_pred = [wl_coeff, np.zeros(20), np.zeros(31), np.zeros(54)]
                points = pywt.waverec(wl_coeff_pred, 'sym5')
                points_pred = np.append(points.real, points.imag).reshape(2, -1).T
                points_pred[:, 0] = points_pred[:, 0] + x_center_pred
                points_pred[:, 1] = points_pred[:, 1] + y_center_pred
                pred_vector = points_pred.flatten()
                pred_vectors = np.append(pred_vectors, pred_vector)

                wl_coeff = real_gt + imag_gt * 1j
                wl_coeff_gt = [wl_coeff, np.zeros(20), np.zeros(31), np.zeros(54)]
                points = pywt.waverec(wl_coeff_gt, 'sym5')
                points_gt = np.append(points.real, points.imag).reshape(2, -1).T
                points_gt[:, 0] = points_gt[:, 0] + x_center_gt
                points_gt[:, 1] = points_gt[:, 1] + y_center_gt
                gt_vector = points_gt.flatten()
                gt_vectors = np.append(gt_vectors, gt_vector)
            pred_vectors = torch.tensor(pred_vectors).cuda().contiguous().view(-1, 100)
            gt_vectors = torch.tensor(gt_vectors).cuda().contiguous().view(-1, 100)

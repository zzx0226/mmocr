...
class TaskDecomposition(nn.Module):
    ...
    def __init__(self, feat_channels, stacked_convs, la_down_rate=8, conv_cfg=None, norm_cfg=None):
        ...
        self.layer_attention = nn.Sequential(nn.Conv2d(self.in_channels, self.in_channels // la_down_rate, 1),
                                             nn.ReLU(inplace=True),
                                             nn.Conv2d(self.in_channels // la_down_rate, self.stacked_convs, 1, padding=0),
                                             nn.Sigmoid())

        self.reduction_conv = ConvModule(self.in_channels,
                                         self.feat_channels,
                                         1,
                                         stride=1,
                                         padding=0,
...
class BS_TOODHead(ATSSHead):
    ...
    def __init__(self,
                 ...
                 initial_loss_cls=dict(type='FocalLoss',
                                       use_sigmoid=True,
                                       activated=True,
                                       gamma=2.0,
                                       alpha=0.25,
        ...
        self.cp_num = cp_num
        super(BS_TOODHead, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            self.inter_convs.append(
                ConvModule(chn, self.feat_channels, 3, stride=1, padding=1, conv_cfg=conv_cfg, norm_cfg=self.norm_cfg))

        self.cls_decomp = TaskDecomposition(self.feat_channels, self.stacked_convs, self.stacked_convs * 8, self.conv_cfg,
                                            self.norm_cfg)
        self.reg_decomp = TaskDecomposition(self.feat_channels, self.stacked_convs, self.stacked_convs * 8, self.conv_cfg,
                                            self.norm_cfg)
        ...
        self.bs_reg = nn.Conv2d(self.feat_channels, self.cp_num * 4, 3, padding=1)

        self.cls_prob_module = nn.Sequential(nn.Conv2d(self.feat_channels * self.stacked_convs, self.feat_channels // 4, 1),
                                             nn.ReLU(inplace=True), nn.Conv2d(self.feat_channels // 4, 1, 3, padding=1))
        self.reg_offset_module = nn.Sequential(nn.Conv2d(self.feat_channels * self.stacked_convs, self.feat_channels // 4, 1),
                                               nn.ReLU(inplace=True), nn.Conv2d(self.feat_channels // 4, 4 * 2, 3, padding=1))
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(zip(feats, self.scales, self.prior_generator.strides)):
            ...
            if self.anchor_type == 'anchor_free':
                ...
                reg_bbox = distance2bbox(self.anchor_center(anchor) / stride[0],
                                         reg_dist).reshape(b, h, w, 4).permute(0, 3, 1, 2)  # (b, c, h, w)
                # bs_dist = scale(self.bs_reg(reg_feat).exp()).float()
                bs_dist = scale(self.bs_reg(reg_feat)).float()
                bs_pred = bs_dist.permute(0, 2, 3, 1)
            elif self.anchor_type == 'anchor_based':
                reg_dist = scale(self.tood_reg(reg_feat)).float()
                reg_dist = reg_dist.permute(0, 2, 3, 1).reshape(-1, 4)
                reg_bbox = self.bbox_coder.decode(anchor, reg_dist).reshape(b, h, w, 4).permute(0, 3, 1, 2) / stride[0]
            else:
                raise NotImplementedError(f'Unknown anchor type: {self.anchor_type}.'
        ...
        return tuple(cls_scores), tuple(bbox_preds), tuple(bs_preds)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, bs_pred, labels, label_weights, bbox_targets, cp_targets,
                    alignment_metrics, stride):
        ...
        bs_pred = bs_pred.permute(0, 2, 3, 1).reshape(-1, 20)
        bbox_targets = bbox_targets.reshape(-1, 4)
        cp_targets = cp_targets.reshape(-1, 20)
        ...
        loss_cls = cls_loss_func(cls_score, targets, label_weights, avg_factor=1.0) / 10
        ...
        if len(pos_inds) > 0:
            ...
            pos_cp_targets = cp_targets[pos_inds]
            pos_bs_pred = bs_pred[pos_inds]

            pos_decode_bbox_pred = pos_bbox_pred
            pos_decode_bbox_targets = pos_bbox_targets / stride[0]

            pos_decode_bs_pred = pos_bs_pred
            pos_decode_cp_targets = pos_cp_targets / stride[0]
            # regression loss
            pos_bbox_weight = self.centerness_target(
                pos_anchors, pos_bbox_targets) if self.epoch < self.initial_epoch else alignment_metrics[pos_inds]

            pos_centerness_targets = self.centerness(pos_bbox_targets)
            ...
            loss_bs = F.smooth_l1_loss(pos_decode_bs_pred, pos_decode_cp_targets, reduction="none")
            loss_bs = ((loss_bs.mean(dim=-1) * pos_centerness_targets).sum() / centerness_denorm)
        else:
            loss_bbox = bbox_pred.sum() * 0
            loss_bs = bs_pred.sum() * 0
            pos_bbox_weight = bbox_targets.new_tensor(0.)

        return loss_cls, loss_bbox, loss_bs, alignment_metrics.sum(), pos_bbox_weight.sum()

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'bs_preds'))
    def loss(self, cls_scores, bbox_preds, bs_preds, gt_bboxes, gt_cp, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        flatten_bs_preds = torch.cat([
            bs_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, self.cp_num * 4) * stride[0]
            for bs_pred, stride in zip(bs_preds, self.prior_generator.strides)
        ], 1)

        cls_reg_targets = self.get_targets(flatten_cls_scores,
                                           flatten_bbox_preds,
                                           flatten_bs_preds,
                                           anchor_list,
                                           valid_flag_list,
        ...
        (anchor_list, labels_list, label_weights_list, bbox_targets_list, cp_targets_list, alignment_metrics_list,
         alignment_metrics_list_cp) = cls_reg_targets
        #anchors_list, labels_list, label_weights_list, bbox_targets_list, norm_alignment_metrics_list,norm_alignment_metrics_list_cp
        losses_cls, losses_bbox,losses_bs,\
        ...
        cp_avg_factor = reduce_mean(sum(bbox_avg_factors)).clamp_(min=1).item()
        losses_bbox = list(map(lambda x: x / bbox_avg_factor, losses_bbox))
        losses_bs = list(map(lambda x: x / cp_avg_factor, losses_bs))
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, losses_bs=losses_bs)
    ...
    def _get_bboxes_single(self,
        ...
        mlvl_scores = []
        mlvl_labels = []
        for cls_score, bbox_pred, priors, stride in zip(cls_score_list, bbox_pred_list, mlvl_priors,
                                                        self.prior_generator.strides):
    ...
    def get_targets(self,
        ...
        if self.epoch < self.initial_epoch:
            (all_anchors, all_labels, all_label_weights, all_bbox_targets, all_bbox_weights, all_cp_targets, all_cp_weights,
             pos_inds_list, neg_inds_list) = multi_apply(super()._get_target_single,
                                                         anchor_list,
                                                         valid_flag_list,
            ...
            all_assign_metrics_cp = [weight[..., 0] for weight in all_cp_weights]
        else:
            (all_anchors, all_labels, all_label_weights, all_bbox_targets, all_cp_targets,
             all_assign_metrics) = multi_apply(self._get_target_single,
                                               cls_scores,
                                               bbox_preds,
                                               bs_preds,
                                               anchor_list,
        ...
        cp_targets_list = images_to_levels(all_cp_targets, num_level_anchors)
        norm_alignment_metrics_list = images_to_levels(all_assign_metrics, num_level_anchors)
        norm_alignment_metrics_list_cp = images_to_levels(all_assign_metrics_cp, num_level_anchors)

        return (anchors_list, labels_list, label_weights_list, bbox_targets_list, cp_targets_list, norm_alignment_metrics_list,
                norm_alignment_metrics_list_cp)
    ...
    def _get_target_single(self,
        ...
        assign_result = self.alignment_assigner.assign(cls_scores[inside_flags, :], bbox_preds[inside_flags, :],
                                                       bs_preds[inside_flags, :], anchors, gt_bboxes, gt_bboxes_ignore,
                                                       gt_labels, self.alpha, self.beta)
        #assign_result = self.assigner.assign(anchors, num_level_anchors_inside, gt_bboxes, gt_cp, gt_bboxes_ignore, gt_labels)
        # assign_result = self.assigner.assign(anchors, num_level_anchors_inside, gt_bboxes, gt_cp, gt_bboxes_ignore, gt_labels)
        assign_ious = assign_result.max_overlaps
        assign_metrics = assign_result.assign_metrics

        sampling_result = self.sampler.sample(assign_result, anchors, gt_bboxes, gt_cp)

        num_valid_anchors = anchors.shape[0]
        bbox_targets = torch.zeros_like(anchors)
        cp_targets = torch.zeros(anchors.shape[0], 20).cuda()
        ...
        if len(pos_inds) > 0:
            ...
            pos_cp_targets = sampling_result.pos_gt_cp
            bbox_targets[pos_inds, :] = pos_bbox_targets
            cp_targets[pos_inds, :] = pos_cp_targets
        ...
        if unmap_outputs:
            ...
            cp_targets = unmap(cp_targets, num_total_anchors, inside_flags)
            norm_alignment_metrics = unmap(norm_alignment_metrics, num_total_anchors, inside_flags)
        return (anchors, labels, label_weights, bbox_targets, cp_targets, norm_alignment_metrics)

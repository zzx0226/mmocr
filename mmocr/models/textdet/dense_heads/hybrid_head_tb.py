...
class HybridHead_tb(HeadMixin, BaseModule):
    def __init__(self,
                 in_channels,
                 scales,
                 bs_degree=4,
        ...
        self.bs_conv_reg = nn.Conv2d(self.in_channels, self.bs_reg, kernel_size=3, stride=1, padding=1)
    ...
    def get_boundary(self, score_maps, img_metas, rescale):
        ...
        # file_num = img_metas[0]['filename'].split("/")[-1].split('.')[0]
        # saveboundaries = np.array(boundaries)
        # np.savetxt("result/" + file_num + ".txt", saveboundaries, fmt='%d', delimiter=',', newline='\r\n')

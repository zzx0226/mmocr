...
class BS_FCOSHead_Att(AnchorFreeHead, HeadMixin):
    def __init__(
            self,
            bs_degree=4,
            cp_num=8,
            ...
            loss_area=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),
            #   bs_degree=4,
            #   cp_num=14),
            postprocessor=dict(
        ...
        self.conv_bs = nn.Conv2d(self.feat_channels, self.cp_num * 2, 3, padding=1)
    ...
    def cal_areas(self, masks):
        ...
        for polygons_per_obj in masks:
            area_per_obj = 0
            area_per_obj += self.polygon_area(polygons_per_obj[0::2], polygons_per_obj[1::2])
    ...
    def forward_single(self, x, scale, stride):
        ...
        cls_score, bbox_pred, bs_pred, centerness = self.forward_single_scale(x)  #, bs_feat
    ...
    def loss(self,
        ...
        flatten_bs_preds = [bs_pred.permute(0, 2, 3, 1).reshape(-1, self.cp_num * 2) for bs_pred in bs_preds]
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, gt_cp, points, regress_ranges, num_points_per_lvl):
        ...
        gt_cp = gt_cp[None].expand(num_points, num_gts, int(self.cp_num * 2)).clone()
        ...
        xs_cp = xs.repeat((1, self.cp_num)).reshape(len(xs), -1, int(len(gt_cp[0][0]) / 2))
        ys_cp = ys.repeat((1, self.cp_num)).reshape(len(ys), -1, int(len(gt_cp[0][0]) / 2))
    ...
    def get_boundary(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            for i in index:
                # split_index = int(self.cp_num / 4)
                # eachContours = bs_cps[i].reshape((-1, 2))
                # TempContours1 = np.vstack((eachContours[split_index:], eachContours[:split_index]))
                ...
                # points1 = bs.bs(uq)
                # points1 = points1[:-1]
                # points = np.append(points1, scores[i])
                # result_list.append(points.tolist())
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, bs_pred,score_factor, priors) in \
            ...
            bs_detections = priors.unsqueeze(1) + bs_pred.view(-1, int(bs_pred.shape[1] / 2), 2)

...
class BS_FCOSHead_tb(AnchorFreeHead, HeadMixin):
    def __init__(
            self,
            bs_degree=4,
            cp_num=5,
        ...
        self.conv_bs = nn.Conv2d(
            self.feat_channels, self.cp_num * 4, 3, padding=1)
    ...
    def loss(self,
        ...
        flatten_bs_preds = [bs_pred.permute(
            0, 2, 3, 1).reshape(-1, self.cp_num * 4) for bs_pred in bs_preds]
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, gt_cp, points, regress_ranges, num_points_per_lvl):
        ...
        if num_gts == 0:
            return gt_labels.new_full((num_points,), self.num_classes), \
                ...
                gt_cp.new_zeros((num_points, self.cp_num*4))
        ...
        gt_cp = gt_cp[None].expand(
            num_points, num_gts, int(self.cp_num * 4)).clone()
        ...
        xs_cp = xs.repeat((1, self.cp_num * 2)
                          ).reshape(len(xs), -1, int(len(gt_cp[0][0]) / 2))
        ys_cp = ys.repeat((1, self.cp_num * 2)
                          ).reshape(len(ys), -1, int(len(gt_cp[0][0]) / 2))
    ...
    def get_boundary(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            for i in index:
                ...
                split_index = int(len(eachContours) / 2)
                TopCP = eachContours[:split_index].reshape(-1, 2)
                BottomCP = eachContours[split_index:].reshape(-1, 2)

                bs = BS_curve(self.cp_num-1, self.bs_degree, cp=TopCP)
                uq = np.linspace(0, 1, self.reconstruction_num)
                bs.set_paras(uq)
                knots = bs.get_knots()
                points1 = bs.bs(uq)

...
class BSNetTargets_bbox(TextSnakeTargets):
    ...
    def generate_cp_maps(self, img_size, text_polys):
        ...
        bbox_map = np.zeros((4, h, w), dtype=np.float32)
        ...
        for poly in text_polys:
            ...
            for i in range(0, cp_num):
                x_map[i, :, :] = mask * cp_coordinates[i, 0] + (1 - mask) * x_map[i, :, :]
                y_map[i, :, :] = mask * cp_coordinates[i, 1] + (1 - mask) * y_map[i, :, :]

            for i in range(0, 4):
                bbox_map[i, :, :] = mask * bbox[i] + (1 - mask) * bbox_map[i, :, :]

        # print(bbox_map.shape)
        return x_map, y_map, bbox_map

    def generate_bbox_maps(self, img_size, text_polys):
        ...
        for poly in text_polys:
            ...
            polygon = np.array(text_instance).reshape((1, -1, 2))
            cv2.fillPoly(mask, polygon.astype(np.int32), 1)
            x, y, w, h = cv2.boundingRect(mask.copy())
            bbox = np.array([x, y, x + w, y + h]).astype(np.float32)
            for i in range(0, 4):
                print((mask * bbox[i]).shape, ((1 - mask) * bbox_map[i, :, :]).shape)
                bbox_map[i, :, :] = mask * bbox[i] + (1 - mask) * bbox_map[i, :, :]
        return bbox_map
    ...
    def generate_level_targets(self, img_size, text_polys, ignore_polys):
        ...
        for ind, size_divisor in enumerate(lv_size_divs):
            ...
            cp_x_maps, cp_y_maps, bbox_maps = self.generate_cp_maps(level_img_size, lv_text_polys[ind])
            current_level_maps.append(cp_x_maps)
            current_level_maps.append(cp_y_maps)

            # bbox_maps = self.generate_bbox_maps(level_img_size, lv_text_polys[ind])

            current_level_maps.append(bbox_maps)

...
class TextSnakeTargets(BaseTextDetTargets):
    ...
    def find_head_tail(self, points, orientation_thr):
        ...
        if len(points) > 4:
            ...
            gaussian = 1. / (np.sqrt(2. * np.pi) * 0.5) * np.exp(-np.power((x - 0.5) / 0.5, 2.) / 2)
            ...
            if self.vector_slope(points[1] - points[0]) + self.vector_slope(points[3] - points[2]) < self.vector_slope(
                    points[2] - points[1]) + self.vector_slope(points[0] - points[3]):
            ...
            vertical_len_sum = norm(points[vertical_edge_inds[0][0]] -
                                    points[vertical_edge_inds[0][1]]) + norm(points[vertical_edge_inds[1][0]] -
    ...
    def draw_center_region_maps(self, top_line, bot_line, center_line, center_region_mask, radius_map, sin_map, cos_map,
                                region_shrink_ratio):
    ...
    def generate_center_mask_attrib_maps(self, img_size, text_polys):
        ...
        for poly in text_polys:
            ...
            self.draw_center_region_maps(resampled_top_line, resampled_bot_line, center_line, center_region_mask, radius_map,
                                         sin_map, cos_map, self.center_region_shrink_ratio)
    ...
    def generate_targets(self, results):
        ...
        (gt_center_region_mask, gt_radius_map, gt_sin_map, gt_cos_map) = self.generate_center_mask_attrib_maps((h, w),
                                                                                                               polygon_masks)

...
class WLNetTargets_direct(TextSnakeTargets):
    ...
    def generate_wl_maps(self, img_size, text_polys):
        ...
        h, w = img_size
        real_map = np.zeros((20, h, w), dtype=np.float32)
        imag_map = np.zeros((20, h, w), dtype=np.float32)
        for poly in text_polys:
            assert len(poly) == 1
            ...
            EachContour = ResamplePoints[:, 0] + 1j * ResamplePoints[:, 1]

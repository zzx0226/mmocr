...
class CurveDataset(CocoDataset):
    ...
    def __init__(self,
        ...
        super().__init__(ann_file, pipeline, classes, data_root, img_prefix, seg_prefix, proposal_file, test_mode,
                         filter_empty_gt)
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann.get('iscrowd', False):
                ...
                gt_bs_cp.append(ann['bs_cp'])

        if gt_bboxes:
            gt_bboxes = np.array(gt_bboxes, dtype=np.float32)
            gt_labels = np.array(gt_labels, dtype=np.int64)
            gt_bs_cp = np.array(gt_bs_cp, dtype=np.float32)
        else:
            gt_bboxes = np.zeros((0, 4), dtype=np.float32)
            gt_labels = np.array([], dtype=np.int64)
            gt_bs_cp = np.array([], dtype=np.float32)
    ...
    def load_annotations_Curve(self, ann_file):
        """Load annotation from COCO style annotation file.
    ...
    def get_ann_info_Curve(self, idx):
        """Get COCO annotation by index.
    ...
    def evaluate(self, results, metric='hmean-iou', logger=None, score_thr=0.3, rank_list=None, **kwargs):
        ...
        # self.data_infos = self.load_annotations_Curve(self.ann_file)

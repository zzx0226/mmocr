...
log_level = 'INFO'
load_from = None
resume_from = 'work_dirs/wlnet_test_rbio28/latest.pth'
workflow = [('train', 1)]
optimizer = dict(type='Adam', lr=0.0001)
optimizer_config = dict(grad_clip=None)
lr_config = dict(policy='step', step=[400, 900, 950])
total_epochs = 1000
wavelet_type = 'rbio2.8'
reconstr_points = 50
model = dict(
    type='WLNet',
    backbone=dict(
    ...
    bbox_head=dict(
        ...
        in_channels=256,
        scales=(8, 16, 32),
        loss=dict(type='WLLoss_test', wavelet_type='rbio2.8'),
        postprocessor=dict(
...
work_dir = './work_dirs/wlnet_test_rbio28'
gpu_ids = range(0, 1)

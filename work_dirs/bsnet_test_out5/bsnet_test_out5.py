...
lr_config = dict(policy='step', step=[200, 600, 600, 1000, 1500])
total_epochs = 1500
cp_num = 8
bs_degree = 4
reconstr_points = 100
model = dict(
    ...
    bbox_head=dict(
        ...
        in_channels=256,
        scales=(4, 8, 16, 32, 64),
        loss=dict(type='BSLoss_Out5', bs_degree=4, cp_num=8),
        postprocessor=dict(
...
leval_prop_range_ctw1500 = ((0, 0.2), (0.15, 0.35), (0.3, 0.6), (0.55, 0.85),
                            (0.8, 1.0))
train_pipeline_ctw1500 = [
    ...
    dict(type='Pad', size_divisor=32),
    dict(
        type='BSNetTargets_Out5',
        bs_degree=4,
        cp_num=8,
        resample_num=100,
        level_proportion_range=((0, 0.2), (0.15, 0.35), (0.3, 0.6),
                                (0.55, 0.85), (0.8, 1.0))),
    dict(
        type='CustomFormatBundle',
        keys=['p2_maps', 'p3_maps', 'p4_maps', 'p5_maps', 'p6_maps'],
        visualize=dict(flag=False, boundary_key=None)),
    dict(
        type='Collect',
        keys=['img', 'p2_maps', 'p3_maps', 'p4_maps', 'p5_maps', 'p6_maps'])
]
    ...
    train=dict(
        ...
        pipeline=[
            ...
            dict(
                type='Collect',
                keys=[
                    'img', 'p2_maps', 'p3_maps', 'p4_maps', 'p5_maps',
                    'p6_maps'
                ])
        ]),
    val=dict(
...
work_dir = './work_dirs/bsnet_test_out5'
gpu_ids = range(0, 1)

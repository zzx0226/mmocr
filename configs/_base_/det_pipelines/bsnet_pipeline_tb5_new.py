...
#     dict(type='BSNetTargets_tb_new', bs_degree=4, cp_num=5, level_proportion_range=leval_prop_range_icdar2015),
...
train_pipeline_icdar2015 = [
    ...
    dict(type='Normalize', **img_norm_cfg),
    dict(type='RandomScaling', size=800, scale=(3. / 4, 5. / 2)),
    dict(type='RandomCropFlip', crop_ratio=0.0, iter_num=1, min_area_ratio=0.2),
    dict(type='RandomCropPolyInstances', instance_key='gt_masks', crop_ratio=0.8, min_side_ratio=0.3),
    dict(type='RandomRotatePolyInstances', rotate_ratio=0.0, max_angle=30, pad_with_fixed_color=False),
    dict(type='SquareResizePad', target_size=800, pad_ratio=0.6),
...
test_pipeline_icdar2015 = [
    ...
    dict(
        ...
        transforms=[
            ...
            # dict(type='Resize', img_scale=(2260, 2260), keep_ratio=True),
            dict(type='Normalize', **img_norm_cfg),
            dict(type='Pad', size_divisor=32),
...
train_pipeline_totaltext = [
    ...
    dict(type='RandomFlip', flip_ratio=0.5, direction='horizontal'),
    dict(type='Pad', size_divisor=32),
    dict(type='BSNetTargets_tb_new', bs_degree=4, cp_num=5, level_proportion_range=leval_prop_range_totaltext),
    dict(type='CustomFormatBundle', keys=['p3_maps', 'p4_maps', 'p5_maps'], visualize=dict(flag=False, boundary_key=None)),
    dict(type='Collect', keys=['img', 'p3_maps', 'p4_maps', 'p5_maps'])
]
